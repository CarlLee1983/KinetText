...
A plain truncated hash is not sufficient. The values KinetiText fingerprints — currently the Microsoft Edge TTS token and its refresh URL, the latter drawn from a handful of plausible endpoints — come from small enough spaces that an unkeyed digest is recoverable by dictionary in seconds. Diagnostic output is designed to be pasted into a bug report or handed to someone else, so a fingerprint in it must not be a disguised copy of the secret.
